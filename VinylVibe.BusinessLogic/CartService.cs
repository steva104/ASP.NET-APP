...
namespace VinylVibe.BusinessLogic
{
	public class CartService : ICartService
	{
		...
		{
			var cartItems = _unitOfWork.ShoppingCart.GetAll(x => x.UserId == userId, includeProperties: "Product");
		...
		public void RemoveCartItem(int cartId)
		{
			...
			if (httpContext != null)
			{
				int newCartCount = _unitOfWork.ShoppingCart.GetAll(x => x.UserId == cartFromDb.UserId).Count() - 1;
		...
		public int ProcessCheckout(string userId, ShoppingCartViewModel cartViewModel)
		{
			...
			viewModel.Items= _unitOfWork.ShoppingCart.GetAll(x => x.UserId == userId, includeProperties: "Product");
			...
			var cartItems = _unitOfWork.ShoppingCart.GetAll(x => x.UserId == userId);
